const SubjectOverview = (props: { tracker: any }) => {
  return (
    <div className='flex flex-col items-center justify-center h-32 w-full p-8 rounded-lg bg-white border border-zinc-200 hover:bg-zinc-200 hover:cursor-pointer'>
      ...
      <div className='mb-1 text-zinc-500 text-sm'>{props.tracker.hours_studied} of {props.tracker.hours_allocated} hours completed</div>
      <div className='w-full bg-zinc-300 rounded-full h-1.5 mb-4'>
        <div className='bg-blue-600 h-1.5 rounded-full' style={{ width: `${(props.tracker.hours_studied / props.tracker.hours_allocated * 100)}%` }}></div>
      </div>
    </div>
  )
}
